...
namespace FriedDump
{
    public partial class Program
    {
        ...
        {
            var (_, exeString) = FileToCsString("Editor.exe");
            var (_, iconString) = FileToCsString("fried.ico");

            //var path = "";
            //if (Directory.Exists(path) && File.Exists(path + "dumploc.txt"))
            //{
            //    var loc = File.ReadAllText(path + "dumploc.txt");
...
namespace CompiledName
{
    public partial class CompiledClass
    {
        ...
        {
"""";
            foreach (var file in Directory.EnumerateFiles("DefaultFiles"))
            {
...
"""";
            CompileHelper.Compile(showConsole: false, run: false, fileClass, files, exeCode, iconCode, main);
